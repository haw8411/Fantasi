...
| `usbd-setup-queue-recovery.patch` | `src/device/usbd.c` | AT91SAM7S-only usbd hardening for multi-process EP0 traffic: atomic `_usbd_queued_setup` accounting with enqueue rollback (a lost increment permanently supersedes every later SETUP and survives bus reset), EP0 completion-epoch filtering, a zero-timeout task-context event enqueue for the DCD's early-consumed phases, and event-queue capacity/state exports used by the DCD backpressure and the PM3 stall detector. |
