...
|---|---|---|
| `dcd_at91sam7s.c` | **new file** → `src/portable/microchip/at91sam7s/` | The AT91SAM7S (ARM7TDMI) UDP device controller. Upstream has no SAM7S DCD. Adapted from `dcd_samg.c`; includes the DPRAM byte-leakage guard (gate UDP IRQ around FDR access; separate OUT/IN passes), the short-packet "stray char" fix (advance by actual `RXBYTECNT`, not epsize), the EP0 phase machine with control-transaction epochs, and the early-consume staging design (an EP0 OUT packet or status ZLP that beats TinyUSB's descriptor is consumed into a fixed 8-byte stage and the bank released, so EP0's interrupt never sits masked while a foreign process's SETUP waits - the multi-process WebUSB mux requirement). |
| `usbd-setup-queue-recovery.patch` | `src/device/usbd.c` | Atomic `_usbd_queued_setup` accounting for AT91SAM7S and nRF5x. AT91SAM7S also gets EP0 completion-epoch filtering, a zero-timeout task-context event enqueue, and event-queue capacity/state exports used by DCD backpressure and the PM3 stall detector. |
| `usbd-status-nrf5x-dma-arbitration.patch` | `src/device/usbd.c`, `src/device/usbd_control.c`, `src/portable/nordic/nrf5x/dcd_nrf5x.c` | Serializes nRF EasyDMA work inside the DCD and hands EP0 status completion directly to the USB task, so the event-queue consumer never blocks while trying to defer work through its own full queue. |
...
| `msc-cache-mode-page.patch` | `src/class/msc/msc_device.c` | Reports the volatile write cache in MODE SENSE(6), so hosts issue SYNCHRONIZE CACHE before treating staged MSC writes as durable. |
